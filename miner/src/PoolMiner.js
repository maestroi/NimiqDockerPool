...
class PoolMiner extends BasePoolMiner {
    /**
    ...
    async _onBlockMined(block, fullValid) {
        ...
        if (readyState === 3) {
            Nimiq.Log.w(PoolMiner, `Incorrect websocket state detected! Reconnecting`);
            this._exponentialBackoffReconnect = 0;
            this._timeoutReconnect();
            return;
        } else {
            Nimiq.Log.i(PoolMiner, `Still connected to pool`);
        }
        this._send({
            message: 'share',
    ...
    _register() {
        ...
        Nimiq.Log.i(PoolMiner, `Registering to pool (${this._host}) using device id ${this._deviceId} (${deviceName}) as a smart client.`);
        this._send({
            message: 'register',
    ...
    changeServer(server, port) {
        Nimiq.Log.w(PoolMiner, `Switching to ${server}:${port}`);
        this.disconnect();
        this.connect(server,port);
    }
}

module.exports = PoolMiner;