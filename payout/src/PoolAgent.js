...
class PoolAgent extends Nimiq.Observable {
    ...
    async _onRegisterMessage(msg) {
        ...
        this._deviceName = msg.deviceLabel || msg.deviceName || '';
        switch (msg.mode) {
        ...
        const bannedAddr = [];
        if(bannedAddr.indexOf(this._address.toUserFriendlyAddress()) !== -1){
           throw new Error("Address not allowed. Contact support!");
           return;
        }

        if(msg.hasOwnProperty('startDifficulty')){
            if(isNaN(msg.startDifficulty)){
                msg.startDifficulty = 1;
            }
            this._difficulty = msg.startDifficulty;
            Nimiq.Log.i(PoolAgent, `Client wants to start at difficulty for address: ${ this._address.toUserFriendlyAddress() } to ${this._difficulty}`);
        }

        Nimiq.Log.i(PoolAgent, `Start difficulty for address: ${ this._address.toUserFriendlyAddress() } to ${this._difficulty}`);
        ...
        await this._pool.registerUserDevice(this._userId, this._deviceId, this._deviceName);
    ...
    async _onSmartShareMessage(msg) {
        ...
        if (invalidReason !== null) {
            Nimiq.Log.i(PoolAgent, `INVALID share from ${this._address.toUserFriendlyAddress()} (smart): ${invalidReason}`);
            this._sendError('invalid share: ' + invalidReason);
            this._countNewError();
            return;
        }
