...
(async () => {
    ...
    if (config.poolServer.enabled) {
        const poolServer = new PoolServer($.consensus, config.pool, config.poolServer.port, config.poolServer.mySqlUsr, config.poolServer.mySqlPsw, config.poolServer.mySqlHost, config.poolServer.sslKeyPath, config.poolServer.sslCertPath);
    ...
    if (config.poolService.enabled) {
        const poolService = new PoolService($.consensus, config.pool, config.poolService.mySqlUsr, config.poolService.mySqlPsw, config.poolService.mySqlHost);
        poolService.start();
    }
    if (config.poolPayout.enabled) {
        ...
        const poolPayout = new PoolPayout($.consensus, wallet || $.wallet, config.pool, config.poolPayout.mySqlUsr, config.poolPayout.mySqlPsw, config.poolPayout.mySqlHost, config.poolPayout.ownerPayoutAddress);
        poolPayout.start();
    }
