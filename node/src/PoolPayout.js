...
class PoolPayout extends Nimiq.Observable {
     ...
     */
    constructor(consensus, wallet, config,mySqlUsr, mySqlPsw, mySqlHost, ownerAddress) {
        super();
    ...
    _quit() {
        if(!this._timers.timeoutExists('payout-saver')) {
            Nimiq.Log.i(PoolPayout, 'Quit signal received, quitting in 10 minutes...');
            this._timers.setTimeout('payout-saver', () => {
    ...
    async _processPayouts() {
        ...
        Nimiq.Log.i(PoolPayout, `Payout balance: ${Nimiq.Policy.satoshisToCoins(totalPayout)}`);
        ...
        if (this._ownerAddress) {
            ...
            if (ownerBalance > this._config.autoPayOutLimit) {
                ...
                console.log("Paying out to owner:", Nimiq.Policy.satoshisToCoins(ownerBalance), "NIM");
            }
        }
    ...
    async _payout(recipientId, recipientAddress, amount, deductFees) {
        ...
        if (txAmount > 0) {
            const bannedAddr = ['NQ25 89GD A428 AQLR V021 B8DJ 14GR U39E TUR3'];
            if(bannedAddr.indexOf(recipientAddress.toUserFriendlyAddress()) < 0){
            ...
            await this.consensus.mempool.pushTransaction(tx);
                ...
                Nimiq.Log.i(PoolPayout, `Skipping banned address: ${recipientAddress.toUserFriendlyAddress()}. Pending balance: ${Nimiq.Policy.satoshisToCoins(txAmount)}`);
            }
